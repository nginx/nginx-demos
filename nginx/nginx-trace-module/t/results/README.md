...
| `t/redact.t`          | M8.0      | 11    | AC-11 / G6 — redaction happens *before* the shm write. `Authorization` and `Cookie` are masked with **no** `trace_redact` configured (the NFR-SEC-3 default-deny set); the header *name* survives so the timeline stays readable; an explicit list redacts custom names, matches case-insensitively, and *adds to* rather than replacing the default set; a watched variable whose name matches is masked too (not only headers); the fixed-width mask leaks no length information; a redacted response header (`Set-Cookie`) never reaches the ring; masking does not corrupt the surrounding JSON; the client response is untouched (G1). |
